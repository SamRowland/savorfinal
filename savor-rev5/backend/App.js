  ...
  app.get("/recipe/id/:id", async (req, res) => {
    ...
    app.delete("/deleteRecipe/:id", async (req, res) => {
        try {
            ...
            await client.connect();
            const deletedRecipeId = req.params.id
            var o_id = new mongoose.Types.ObjectId(deletedRecipeId);
            const query = {"_id": o_id};
            const deletedRecipe = await db.collection("recipes").findOne(query);
            const result = await db.collection("recipes").deleteOne(query);
             query = deletedRecipe.author;
            var updateData = { $inc: {"totalLikes": 0 - deletedRecipe.likes}};
            const incAuthorLikes = await db.collection("users").updateOne(query, updateData);
            updateData = { $pull: {"likedRecipes": deletedRecipeId}}
            const removeRecipeFromLikes = await db.collection("users").updateOne(query, updateData);
            res.status(200);
            res.send();
        } catch {
            console.error("An error occurred:", error);
